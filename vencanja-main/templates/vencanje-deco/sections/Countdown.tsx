...
const Countdown: FC<Props> = ({ section, event }) => {
  ...
  return (
    <section id={id} className="vd-section-tight">
      <div className="vd-shell text-center">
        <p className="vd-eyebrow">Odbrojavanje</p>
        {data.title ? (
          <h2 className="vd-display-sm mt-4">{data.title}</h2>
        ) : null}
        <p className="vd-meta mt-4">
        ...
        <motion.div
          ...
          className="vd-count mx-auto mt-10 max-w-2xl"
        >
          {cells.map((cell) => (
            <div key={cell.label} className="vd-count-cell">
              <span className="vd-count-num">
                {String(cell.value).padStart(2, "0")}
              </span>
              <span className="vd-count-label">{cell.label}</span>
            </div>
          ))}
        </motion.div>

        {data.description ? (
          <p className="vd-body mx-auto mt-8 max-w-md">{data.description}</p>
        ) : null}
      </div>
    </section>
  );
