...
const Countdown: FC<Props> = ({ section, event, theme }) => {
  ...
  const items = [
    ...
    { label: "Sek", value: timeLeft.seconds, color: "#FFB703" },
  ];

  return (
    <section id={id} className="bday-section overflow-hidden">
      <div className="bday-shell">
        ...
        <div
          ...
          style={{
            background: `linear-gradient(135deg, ${accent}18, ${secondary}18 50%, ${mint}18)`,
          }}
        >
          {data.imageUrl ? (
            <img
          ...
          <div className="relative z-10 grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
            {items.map((item, index) => (
              <motion.div
                ...
                className="bday-card-strong px-3 py-6 text-center sm:py-8"
              >
                <p
                ...
                <p className="mt-2 text-[11px] font-semibold uppercase tracking-[0.24em] text-black/45">
                  {item.label}
                </p>
              </motion.div>
            ))}
