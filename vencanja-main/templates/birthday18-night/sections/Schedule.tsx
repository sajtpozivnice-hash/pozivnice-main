"use client";

import { FC } from "react";
import { motion } from "framer-motion";
import { ScheduleSection } from "@/types/sections";
import { EventConfig, ThemeConfig } from "@/types/config";

type Props = {
  section: ScheduleSection;
  event: EventConfig;
  theme: ThemeConfig;
};

/** Neon vertical spine: a glowing center timeline with items alternating left/right, not a left-rail list. */
const Schedule: FC<Props> = ({ section }) => {
  const { data, id } = section;
  const items = [...(data.items || [])].sort((a, b) =>
    a.time.localeCompare(b.time),
  );

  return (
    <section id={id} className="bn-section overflow-hidden">
      <div className="bn-shell relative z-10">
        <div className="text-center">
          <p className="bn-label mb-3">{data.subtitle || "REDOSLED PROGRAMA"}</p>
          <h2 className="bn-display text-5xl sm:text-6xl lg:text-7xl">
            {data.title || "VEČE"}
          </h2>
        </div>

        <div className="bn-spine">
          <div className="bn-spine__line" aria-hidden />
          {items.map((item, index) => (
            <motion.article
              key={item.id}
              initial={{ opacity: 0, y: 16 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: index * 0.06 }}
              className={
                "bn-spine__item" +
                (index % 2 === 0 ? " bn-spine__item--left" : " bn-spine__item--right")
              }
            >
              <span className="bn-spine__dot" aria-hidden />
              <p className="bn-schedule__time bn-mono">{item.time}</p>
              <h3 className="bn-schedule__title">{item.title}</h3>
              {item.description ? (
                <p className="mt-1 max-w-xs text-sm text-[color:var(--bn-muted)]">
                  {item.description}
                </p>
              ) : null}
            </motion.article>
          ))}
        </div>
      </div>
    </section>
  );
};

export default Schedule;
